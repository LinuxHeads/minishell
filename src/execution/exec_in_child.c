...
/*   Updated: 2025/02/12 00:48:33 by abdsalah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** - If a pipe was created, uses its write end as STDOUT if out_fd was not redirected.
** - Duplicates out_fd to STDOUT.
** - Closes the previous file descriptor (if any) and both pipe ends (since they are
**   no longer needed after dup2).
*/
static void child_redirect_fds(int *in_fd, int *out_fd, int pipe_created,
...
** - If a new pipe was created, closes its write end and updates prev_fd to its read end.
...
** It sets up the proper file descriptors (handling pipes and redirections), forks,
** and then in the child:
**  - Resets signals.
**  - Validates the command.
**  - Sets up I/O redirections.
**  - Executes the command (or built-in).
** In the parent, it cleans up file descriptors and prepares for the next command in the pipeline.
*/
                   ...
                   char **argv, int redir_flag, int *prev_fd)
{
    ...
    *pid = fork();
    ...
    if (*pid == 0)
    {
        ...
        if (redir_flag)
            exit_error(in_fd, out_fd, pipe_created, pipe_fd);
        child_redirect_fds(in_fd, out_fd, pipe_created, pipe_fd, *prev_fd);
        child_run_command(shell, argv, pipe_created, pipe_fd, in_fd, out_fd, *prev_fd);
    }
