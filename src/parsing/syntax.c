...
/*   Updated: 2025/02/10 03:10:55 by abdsalah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Extended check_redirections:
   ...
   - For each redirection operator, ensures the following token exists and is not an operator */
int check_redirections(t_exec *parser)
{
    ...
    while (parser->commands[cmd_index])
    {
        ...
        while (cmd->tokens[i])
        {
            ...
            {
                if (!cmd->tokens[i + 1] ||
                    is_redirection_operator(cmd->tokens[i + 1]->value) ||
                    !ft_strcmp(cmd->tokens[i + 1]->value, "|"))
                {
